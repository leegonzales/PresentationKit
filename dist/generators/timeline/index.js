 ...
 */
import { alignCaptionsToAudio, estimateCaptionTimings, scaleCaptionsToAudioDuration, } from './caption-sync.js';
 ...
 * ```
 */
export function buildTimeline(talkTrack, audioManifest, options) {
 ...
 */
function buildCaptions(text, audioEntry) {
 ...
 */
export function calculateTotalFrames(timeline) {
 ...
 */
export function getSlideAtTime(timeline, time) {
    return timeline.slides.find((slide) => time >= slide.startTime && time < slide.endTime);
}
/**
 ...
 */
export function getSlideAtFrame(timeline, frame) {
 ...
 */
export function getCaptionAtTime(slide, time) {
    // Convert absolute time to slide-relative time
    const relativeTime = time - slide.startTime;
    return slide.captions.find((caption) => relativeTime >= caption.startTime && relativeTime < caption.endTime);
}
/**
