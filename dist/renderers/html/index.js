 ...
 */
function getSectionColor(sectionId, sections, defaultColor) {
 ...
 */
function prepareSlides(talkTrack, timeline, options) {
    const slides = [];
 ...
 */
function extractSections(slides, talkTrackSections) {
    const sections = [];
    let currentSection = '';
    ...
    for (let i = 0; i < slides.length; i++) {
        ...
        if (slide.section !== currentSection) {
            ...
            if (currentSection && currentCount > 0) {
                const sectionDef = talkTrackSections.find((s) => s.name === currentSection || s.id === currentSection);
                sections.push({
 ...
 */
function prepareMetadata(talkTrack, slides, timeline) {
 ...
 */
function mergeOptions(talkTrack, options) {
 ...
 * ```
 */
export async function renderHtmlPresentation(talkTrack, timeline, outputPath, options) {
 ...
 * @returns HTML string
 */
export function renderHtmlPresentationToString(talkTrack, timeline, options) {
    const opts = mergeOptions(talkTrack, options);
    const slides = prepareSlides(talkTrack, timeline, opts);
    const sections = extractSections(slides, talkTrack.sections);
    const metadata = prepareMetadata(talkTrack, slides, timeline);
 ...
 */
export function getHtmlPresentationSummary(talkTrack, timeline) {
